...
const Header = () => {
  ...
  const searchResult = async (val) => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SEARCH_URL}/api/location/search?q=${val}`
  ...
  const FetchLatandLat = async (id) => {
    if (id === "") return; 
    const res = await fetch(
      `${import.meta.env.VITE_SEARCH_URL}/api/location/details?place_id=${id}`
    );
    const data = await res.json();
    ...
    setSearchData("");
    setAddress(data.data?.formatted_address);
  };
  ...
  return (
    <>
      {/* SEARCH TOGGLE */}
      <div
        className="fixed top-0 left-0 w-full h-screen z-[9999] black-overlay transition-opacity duration-300"
        onClick={showTogle}
        style={{
          opacity: togle ? 1 : 0,
      ...
      >
        <div
          ...
          className="bg-white h-full w-[530px] absolute duration-[500ms] ease-in-out z-40 transition-all"
          style={{
            left: togle ? "0%" : "-100%"
          }}
        >
          <div className="h-screen w-full flex items-center flex-col z-40 overflow-y-auto">
            <div className="sticky top-0 pt-10 pb-6 z-50 bg-white">
              <div className="mb-7">
                <MdClose className="text-[30px]" onClick={showTogle} />
              </div>
              ...
              <div className="sticky top-0 z-50 bg-white border-1 border-[rgba(2,6,12,0.3)] p-4 w-[400px] transition-shadow duration-200 focus-within:shadow-[0_0_10px_rgba(0,0,0,0.5)]">
                <input
                  ...
                  className="text-[rgba(2,6,12,0.5)] border-none outline-none text-[20px] w-[400px]"
                  value={SearchData}
                  onChange={(e) => {
            ...
            <div className="m-10 gap-10 flex flex-col">
                ...
                <ul>
                  {SearchRes &&
                    SearchRes.map((item, index) => (
                      <div className="flex p-2 mt-2 group" key={index}>
                        <MdMyLocation className="text-[22px] mt-4" />
                        <Link to={"/"}>
                          <li
                            className="w-full p-5 border-black border-b-1 border-dashed hover:text-orange-500 cursor-pointer text-[17px] font-[700] text-[#02060c]"
                            onClick={() => {
                              FetchLatandLat(item.place_id);
                              setSearchRes([]);
                              HandleSelect(item);
                              showTogle();
                            }}
                          >
                            ...
                            <ul className="text-[14px] font-[700] text-[rgba(2,6,12,.5)]">
              ...
              <div className="bg-white border-1 border-[rgba(2,6,12,0.3)] p-2 h-[120px] w-[400px] flex justify-start items-center group cursor-pointer">
                <div className="w-[300px] h-[120px] flex justify-start items-center gap-4">
                  <div className="h-[40px] text-[rgba(2,6,12,0.6)]">
                    <MdMyLocation className="text-[25px]" />
                  </div>
                  <div>
                    <div className="text-black group-hover:text-orange-500 font-bold font-serif custom-text text-2xl">
                      <a href="https://www.google.com/">Get Current Location</a>
                    </div>
                    <div className="text-[18px] text-[rgba(2,6,12,0.5)]">
                      Using GPS
                    </div>
                  </div>
                </div>
              ...
              <div className="border-1 border-[rgba(2,6,12,0.3)] w-[400px] group">
                <div className="text-[15px] flex mt-10 justify-center text-[rgba(2,6,12,0.5)] font-bold h-10">
                  RECENT SEARCHES
                </div>
                    ...
                    {resentSearch.slice(0, 5).map((item, index) => (
                      <div key={index} className="flex p-2 gap-2">
                        ...
                        <div className="border-b-1 group p-8 border-dashed border-black w-full">
                          <Link to={"/"}>
                            <li
                              className="cursor-pointer list-none hover:text-orange-500"
                              onClick={() => {
                                FetchLatandLat(item.place_id);
                                setSearchRes([]);
                                HandleSelect(item);
                            ...
                            >
                              <ul className="text-[17px] font-[700] text-[#02060c]">
                                {item?.structured_formatting?.main_text}
                              </ul>
      ...
      >
        ...
        >
          <div className="h-screen w-full flex flex-col gap-3 z-40 overflow-y-auto">
            <div className="sticky top-0 pt-10 pb-6 z-50 bg-white">
              <div className="ml-24">
                <MdClose className="text-[30px]" onClick={loginTogle} />
              </div>
            </div>
      ...
      <header className="fixed top-0 left-0 w-full h-[80px] bg-white z-50 shadow">
        <div className="mx-auto max-w-[1400px] flex items-center justify-between gap-2">
          ...
          <div className="ml-40 max-w-[400px] hidden md:block ">
            <p onClick={showTogle} className="flex cursor-pointer items-center group gap-1 p-2">
          ...
          <nav className="sm:w-[90%] lg:w-[80%] pr-7 flex list-none text-[20px] font-bold h-[80px] justify-end mr-10 items-center gap-10 w-[900px]">
            {Links.map((even, index) =>
              even.name === "Sign In" ? (
                <div onClick={loginTogle} key={index}>
                  <li
                    onMouseEnter={() => setHoveredIndex(index)}
                    onMouseLeave={() => setHoveredIndex(null)}
                    className="relative flex items-center gap-2 py-4 cursor-pointer"
                  >
                    {even.name === "Cart" && (
                      <p
                        className={`
                          ...
                          ${CardData.length > 9 ? "left-4" : "left-5"}
                          ${hoveredIndex === index ? "text-[#ff5200]" : "text-[#4a7212]"}
                        `}
                      >
                    ...
                    <i
                      className={`
                        ${hoveredIndex === index ? "text-[#ff5200]" : ""}
                        ${even.name === "Cart" && CardData.length > 0 ? "text-[#1ba672]" : ""}
                      `}
                    >
                      {even.icon}
                    </i>
                    ...
                    <p
                      ...
                      `}
                    >
                      {userData ? userData.name.split(" ")[0] : even.name}
                    </p>
                  </li>
                </div>
              ) : (
                <Link to={even.path} key={index}>
                  <li
                    onMouseEnter={() => setHoveredIndex(index)}
                    onMouseLeave={() => setHoveredIndex(null)}
                    className="relative flex items-center gap-2 py-4 cursor-pointer"
